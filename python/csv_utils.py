...
def get_data_path(filename):
    ...
    return Path('/'.join(current_path[0:-1]) + 'Data/' + filename)

def get_pandas(filename, encoding='LATIN-1', sep = ';'):
    return pd.read_csv(open(get_data_path(filename)), encoding = encoding, sep = sep)

def get_deliverables_path():
    current_path = os.getcwd().split('/')
    return Path('/'.join(current_path[0:-1]) + 'deliverables' + '/')
